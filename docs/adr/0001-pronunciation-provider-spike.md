# ADR 0001: Historical Azure pronunciation assessment spike

## Status

Historical prototype. The provider-neutral non-production evaluation harness remains, but the Azure runtime adapter and server configuration were retired. Azure Speech Pronunciation Assessment was never proven reliable for deductions.

## Decision

The prototype used a provider-neutral contract before its vendor adapter. Its guarded adapter targeted Azure Speech Pronunciation Assessment behind a server-only boundary and accepted only the documented short-audio formats. That adapter no longer exists in the application runtime. The retained harness makes no provider calls and creates no production pronunciation deductions.

The prototype locale was conservatively limited to `en-US`. It requested the Comprehensive dimension because the REST response exposes word error types only in that dimension, with phoneme granularity and without enabling prosody or content assessment. Aggregate dimensions returned alongside the required word evidence were ignored. Provider words were sequence-aligned to the stored reference transcript; insertions, omissions, and substitutions remained lexical outcomes rather than pronunciation findings.
...
Azure was selected for the prototype because its official documentation describes both scripted assessment with reference text and unscripted assessment without one, plus word and phoneme evidence where supported. The retired adapter used a directly mockable HTTP transport verified against the official short-audio REST documentation. Audio and reference text would be sensitive response data and still require retention, region, processor, and user-consent review before any future live use.
