...
const V3_AUDIO = {
  ...
  metrics: {
    pace: metric('pace'),
    paused_time: metric('paused_time'),
...
function adminClient(
  ...
  concurrent: Record<string, unknown> | null = null,
) {
  const reads = [
...
describe('score route current-generation integrity', () => {
  it('returns an exact stored current snapshot without evaluating or mutating it', async () => {
    const admin = adminClient(
      attempt({ status: 'done', score: 74, section_scores: V3_SCORE, content_result: V3_CONTENT }),
    )
  ...
  it('scores and persists an active current attempt', async () => {
    ...
    expect(mocks.transitionOwnedAttempt).toHaveBeenCalledWith(
      ...
      ['scoring'],
      'done',
      expect.objectContaining({ score: 74, section_scores: V3_SCORE }),
    )
  })
  ...
  it('rejects an abandoned terminal upload before scoring', async () => {
    const admin = adminClient(
      attempt({ status: 'failed', failure_code: ATTEMPT_FAILURE_CODES.clientUploadAbandoned }),
    )
    mocks.authenticatedAttemptContext.mockResolvedValue({ userId: USER_ID, admin })
    const response = await POST(request())
    expect(response.status).toBe(409)
