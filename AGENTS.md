...
- Keep changes compatible with historical `attempts` data. Preserve stored prompt, transcript, capture, scoring-result, and result-snapshot data. Every new v2-scored attempt must store a rubric and score version. Legacy attempts may have null or legacy metadata, and their stored snapshots remain authoritative; later changes must not overwrite or silently reinterpret them. Metrics and content results are JSONB on purpose, and disputes are reapplied on read rather than written into the original result.

## Scoring and Capture

- The 50/50, ten-metric scoring and its Deepgram and DeepSeek provider rules below are the current legacy v1 implementation retained during the v2 transition. Do not treat them as the v2 category architecture.
