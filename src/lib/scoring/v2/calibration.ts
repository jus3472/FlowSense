...
export const CALIBRATION_FIXTURES: readonly CalibrationFixture[] = [
  ...
  },
  {
    ...
    content: content(),
    confidence: 0.45,
    pronunciation: pronunciation('unclear'),
  },
  {
...
function pronunciation(word: string): PronunciationEvaluation {
  return {
    contractVersion: 'v1',
...
function words(text: string, confidence = 0.95): TranscriptWord[] {
  return text
    .replaceAll(',', '')
    .replaceAll('.', '')
    .split(' ')
    .map((word, index) => ({
      word,
      start: 0.4 + index * 0.55,
      end: 0.75 + index * 0.55,
      confidence,
    }))
}
function capture(items: readonly TranscriptWord[], pitch = 180): CaptureMetrics {
  ...
  return {
    ...
    amplitude: Array.from({ length: 181 }, (_, i) => ({ t_ms: i * 50, rms: 0.08 })),
    pitch: Array.from({ length: 181 }, (_, i) => ({ t_ms: i * 50, hz: pitch + (i % 2) * 20 })),
  }
}

export function runCalibrationFixture(fixture: CalibrationFixture): V2ScorePayload {
  const transcriptWords = words(fixture.transcript, fixture.confidence)
  const recording = capture(transcriptWords, fixture.pitch)
...
export const CALIBRATION_BASELINES: Readonly<Record<CalibrationLabel, CalibrationBaseline>> = {
  ...
  'structured-filler-heavy': baseline(['fluency:scored:16', ...standard.slice(1)]),
  monotone: baseline(standard),
  rushed: baseline(standard),
  'slow-long-pauses': baseline(standard),
  ...
  'unclear-pronunciation': baseline(standard),
