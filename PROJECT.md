...
Amber transcript marks identify specific issues, including small issues that do not reduce displayed points. Positive evidence is not highlighted. Whole-response metrics such as Answered the Prompt, Specificity, and Structure do not create transcript marks. Tooltips use the same metric and check labels as the dropdowns. Check symbols distinguish no issue, an issue affecting the score, an issue with no displayed points lost, and unavailable evidence. Each content finding is shown once in its check: a quoted finding must not repeat in its grouped span list. The statistics count shown to users must visibly match the units in the displayed list.

The active visual system is token-based in `src/app/globals.css`: a restrained water-toned light and dark palette, white or dark surfaces, cyan accent, warm amber highlight, Inter body text, Sora display text, and JetBrains Mono for measurements. Responsive form, column, reading, and page widths keep prose focused while allowing Home, Progress, and navigation to use intentional desktop layouts. Rounded white or dark surfaces use quiet borders and restrained shadows, with consistent page rhythm and semantic focus states. Components use semantic Tailwind tokens only. The tests reject component hex values, Tailwind color scales, invalid spacing, insufficient contrast, and nonliteral score-bar behavior.
